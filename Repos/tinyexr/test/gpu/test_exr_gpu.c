/*
 * TinyEXR - CUDA GPU backend test.
 *
 * Validates the GPU decode/encode/processing/resize paths against the CPU
 * reference on real openexr-images. Exits 77 (skip) when no CUDA device is
 * present so it is a no-op on machines / CI without a GPU.
 *
 * Usage: test_exr_gpu [openexr-images-dir]   (default: ~/work/openexr-images)
 *
...
/* ---- decode parity ------------------------------------------------------- */
static void test_decode(exr_gpu_context *ctx, const char *rel) {
    char *p = path_of(rel);
    exr_image cpu, gpu;
    exr_result rc;
    ...
    rc = exr_gpu_load_from_file(ctx, p, NULL, &gpu);
...
static void test_encode(exr_gpu_context *ctx, const char *rel) {
    char *p = path_of(rel);
    exr_image orig, rt;
    void *blob = NULL;
    size_t blob_size = 0;
    ...
    rc = exr_gpu_save_to_memory(ctx, &blob, &blob_size, NULL, &orig,
                                EXR_COMPRESSION_ZIP);
...
/* ---- resize: GPU vs CPU -------------------------------------------------- */
static void test_resize(exr_gpu_context *ctx, const char *rel) {
    char *p = path_of(rel);
    exr_image img;
    ...
    for (fi = 0; fi < 4; ++fi) {
        ...
        rc = exr_gpu_resize_float(ctx, rgba, w, h, 0, gpu_out, dw, dh, 0, ch,
                                  filters[fi], EXR_EDGE_CLAMP, -1);
...
static void test_extract_channel(exr_gpu_context *ctx, const char *rel) {
    char *p = path_of(rel);
    exr_image img;
    const exr_part *pt;
    int c = -1, i;
    ...
    rc = exr_gpu_convert_pixels(ctx, got, EXR_PIXEL_FLOAT, pt->images[c],
...
static void test_combine_channels(exr_gpu_context *ctx, const char *rel) {
    char *p = path_of(rel);
    exr_image img;
    const exr_part *pt;
    ...
    rc = exr_gpu_part_to_rgba_float(ctx, NULL, pt, &gpu, &ow, &oh, &oc);
...
static void test_processing(exr_gpu_context *ctx) {
    size_t n = 4096, i;
    int ch = 4;
    ...
    if (EXR_OK(rc)) {
        ...
        rc = exr_gpu_color_apply_matrix(ctx, gpu, src, n, ch, m);
    ...
    rc = exr_gpu_encode_transfer(ctx, gpu, src, n * ch, EXR_TF_SRGB);
    ...
    rc = exr_gpu_tonemap_float(ctx, gpu, src, n, ch, EXR_TONEMAP_ACES, NULL);
...
int main(int argc, char **argv) {
    ...
    if (!exr_gpu_available()) {
        printf("SKIP: no CUDA device / NVRTC available\n");
        return 77;
    }
    ...
    if (!EXR_OK(rc)) {
        printf("FAIL: exr_gpu_context_create: %s\n", exr_result_string(rc));
        return 1;
    }
    printf("images dir: %s\n", g_dir);
